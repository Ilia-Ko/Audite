...
print(f"1. Read {len(linkEntries)} links from folder '{linkDir}'")
...
print(f"2. Read {len(playEntries)} entries from playlist '{m3uFile}'")
...
def MakeLinkName(audioPath: str) -> str:
...
if modeL2M:
    ...
    print(f"3. Resolving the links from '{linkDir}' ...", end=' ')
    inFiles = []
    nBroken = 0
    ...
    aborted = []
    print(f"4. Appending {len(inFiles)} links into playlist '{m3uFile}' ...", end=' ')
    ...
    if sortM3U:
        res, err = proc.Popen(["sort", m3uFile, "-o", m3uFile], stdout=proc.PIPE, stderr=proc.PIPE, text=True).communicate()
        if len(err) > 0:
    ...
    print(f"3. Converting {len(playEntries)} entries from '{m3uFile}' into local relative links ...", end=' ')
    nAdd = 0
    nErr = 0
    nDup = 0
    missed = []
        ...
        if os.path.isfile(locPath):
            ...
            if doCreate:
                ...
                if len(do) > 0:
                    ...
                    continue
                # ~ err = ""
                res, err = proc.Popen(["ln", "-sr", locPath, lnkPath], stdout=proc.PIPE, stderr=proc.PIPE, text=True).communicate()
                if len(err) == 0:
                    nAdd += 1
